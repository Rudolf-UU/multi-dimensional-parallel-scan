...
fn run_columnwise2(_workers: &Workers, task: *const TaskObject<Data>, loop_arguments: LoopArguments) {
  ...
  |block_index| {
    ...
    indexer = block_index as usize;

    adaptive_lookback(data, column_idx, start, end, indexer);
  },
  // Column-wise block indexing
  |block_index, rows_completed| {
    ...
    // if block_index <3000 {
    //   println!("you're in, {:?}, {:?}, {:?}, {:?}, {:?}", block_index, rows_completed, new_inner_rows, indexer, row_idx, column_idx );
    // }
...
fn adaptive_lookback(data:&Data<'_>, column_idx:usize, start:usize, end:usize, indexer:usize) {
  let aggregate_start = if column_idx == 0 {
    Some(0)
  }  else {
    let previous = indexer - 1;
  ...
  if let Some(aggregate) = aggregate_start {
    ...
    loop {
      ...
      if previous_state == STATE_PREFIX_AVAILABLE {
        ...
        //println!("wrong: {:?}, {:?}", indexer, previous);
        // Continue looping until the state of the previous block changes.
      }
    }
